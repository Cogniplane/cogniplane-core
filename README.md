...
**Multi-tenant agent platform for running governed AI agents at scale, with managed integrations, approvals, and audit logging.**

Running production agent platforms is harder than running a chat UI. You need per-session sandboxes, an MCP gateway that doesn't leak tenant credentials, human-in-the-loop approvals on tool calls, multi-tenant isolation that survives a query that forgets to filter by tenant, and an audit trail your security team can actually read. Cogniplane Core ships all of that as one cohesive system — built on LangChain's [Deep Agents](https://reference.langchain.com/javascript/deepagents) runtime: wire in your MCP servers and skills, and you have a governed agent backend.
...
The runtime needs at least one model-provider API key — Anthropic, OpenAI, Google, OpenRouter, or Z.AI (set the matching `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` / `GOOGLE_API_KEY` / `OPENROUTER_API_KEY` / `ZAI_API_KEY`; the model picker shows only providers you've configured). For Anthropic, use a paid API key from [console.anthropic.com](https://console.anthropic.com) — consumer Claude Free/Pro/Max subscriptions are not supported. You also need an `E2B_API_KEY` + `E2B_TEMPLATE_ID` for sandboxed code execution (`make e2b-build` builds the template in your E2B account). Set them in `apps/backend/.env`.

See [docs/getting-started.md](docs/getting-started.md) for the full walkthrough — including per-tenant key configuration, provider links, and troubleshooting.
...
flowchart TD
    ...
    Agent[Deep Agents loop<br/>in-process, LangGraph]
    Sandbox[Per-session E2B sandbox<br/>code execution, lazy]
    ...
    API -->|stream turn| Agent
    Agent -->|shell + file tools| Sandbox
    Agent -->|JSON-RPC over /mcp/:serverId| MCP
    MCP --> Tools
    API <--> DB
    MCP --> DB
```

The agent loop runs in the backend (LangChain [deepagentsjs](https://reference.langchain.com/javascript/deepagents)); shell and file tools execute in a lazy per-session E2B sandbox. Tool calls go through a tenant-scoped MCP gateway that injects credentials from compiled tenant config — the model never sees raw secrets. Postgres Row-Level Security is the second line of defense behind explicit `tenant_id` filtering.

## What's in the box

- **A production Deep Agents runtime.** Planning, subagents, human-in-the-loop interrupts, and durable LangGraph checkpointing (conversations survive process restarts), wired to sandboxed code execution and a governed tool gateway.
...
- **Skills as content.** Authored as structured operating documents (`SKILL.md` bundles), versioned as immutable revisions, and served to the agent as a read-only `/skills/` file library (native progressive disclosure — the model reads each skill on demand rather than having them all inlined into the prompt). Skill bundles can live on local disk or in S3.
...
`pnpm` workspace · `Node.js 24 LTS` · `Fastify` + `Next.js` · `Postgres` (RLS) · `Redis` (optional) · `WorkOS` (optional) · `E2B` sandboxes · [`deepagents`](https://reference.langchain.com/javascript/deepagents) + LangGraph.
