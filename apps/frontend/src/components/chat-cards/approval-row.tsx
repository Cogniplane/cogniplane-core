...
import type { ApprovalDecision, ApprovalRow } from "./chat-cards.types";
...
export function ApprovalRowView({
  ...
  return (
    <article
      role="alert"
      className="card-enter rounded-lg border border-warning/30 border-l-4 border-l-warning bg-warning-surface/20 px-4 py-3 shadow-sm"
    >
            ...
            expandable ? (
              ...
              >
                ...
                <AnimatedHeight className="min-w-0 flex-1">
                  <span
                    className={`${row.summary.trimStart().startsWith("{") ? "font-mono text-[0.7rem]" : ""} ${
