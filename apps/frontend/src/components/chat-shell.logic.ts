...
import type { Message, Model, ReasoningSegment, Session, ToolResult } from "@cogniplane/shared-types";
...
export function toAguiInitialMessages(messages: Message[]): AGUIMessage[] {
  ...
  for (const message of messages) {
    ...
    // when segments are absent (legacy/RuntimeEvent rows) do we fall back to the
    // single reasoning block rendered ahead of the body.
