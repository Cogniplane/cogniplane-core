...
import { formatRelativeTime, shortId, statusPillClass } from "../../../lib/admin-pii-utils";
...
import { PILL_GRAY, PILL_BLUE, PILL_RED, HINT, SECTION_LABEL, LIST_ITEM } from "../../../lib/ui-tokens";
...
const LIST_ITEM_LINK = `block ${LIST_ITEM} no-underline text-inherit transition-colors hover:border-outline`;
        ...
        {recentQuery.isLoading ? (
          ...
          <div className="flex flex-col gap-2">
            {data.rows.map((row) => {
              ...
              // 404.
              return sessionHref ? (
                <Link href={sessionHref} key={row.scanRunId} className={LIST_ITEM_LINK}>
                  {inner}
                </Link>
              ) : (
                <div
