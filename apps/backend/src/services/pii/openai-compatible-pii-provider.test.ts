...
import {
  OpenAiCompatiblePiiProvider,
  OpenAiCompatiblePiiProviderError
} from "./openai-compatible-pii-provider.js";
...
function buildProvider(fetchImpl: typeof fetch, overrides: Partial<ConstructorParameters<typeof OpenAiCompatiblePiiProvider>[0]> = {}) {
  return new OpenAiCompatiblePiiProvider({
...
test("detectText returns normalized findings from PII model JSON response", async () => {
...
test("transformText throws when transformedText is missing but findings are present", async () => {
  ...
  expect((error as OpenAiCompatiblePiiProviderError).code).toBe("transform_missing_output");
});
...
test("callChat throws invalid_json when model output is not parseable", async () => {
  ...
  const error = await provider.detectText({ text: "hi", entityTypes: ["email"] }).catch((e: unknown) => e);
  expect(error instanceof OpenAiCompatiblePiiProviderError).toBeTruthy();
  expect((error as OpenAiCompatiblePiiProviderError).code).toBe("invalid_json");
});
...
test("callChat surfaces provider_error when payload contains an error object", async () => {
  ...
  const error = await provider.detectText({ text: "hi", entityTypes: ["email"] }).catch((e: unknown) => e);
  expect(error instanceof OpenAiCompatiblePiiProviderError).toBeTruthy();
  expect((error as OpenAiCompatiblePiiProviderError).code).toBe("provider_error");
});
...
test("breaker_open: provider rejects calls when the breaker disallows", async () => {
  ...
  const error = await provider.detectText({ text: "hi", entityTypes: ["email"] }).catch((e: unknown) => e);
  expect(error instanceof OpenAiCompatiblePiiProviderError).toBeTruthy();
  expect((error as OpenAiCompatiblePiiProviderError).code).toBe("breaker_open");
