...
export interface OpenAiCompatiblePiiProviderOptions {
  ...
  fetch?: typeof fetch;
  /**
   * Optional extra request headers. OpenRouter uses `HTTP-Referer`/`X-Title`
   * for attribution; an in-VPC vLLM server ignores unknown headers. Wire
   * whatever the configured endpoint wants.
   */
...
export class OpenAiCompatiblePiiProviderError extends Error {
  readonly code: string;

  constructor(code: string, message: string) {
    super(message);
    this.name = "OpenAiCompatiblePiiProviderError";
    this.code = code;
  }
}
...
/**
 * PII provider backed by any OpenAI-compatible `/chat/completions` endpoint.
 * In Cogniplane Cloud this points at an in-VPC, Cogniplane-operated Gemma
...
export class OpenAiCompatiblePiiProvider implements PiiProvider {
    ...
    if (typeof raw.transformedText !== "string") {
      if (findings.length > 0) {
        throw new OpenAiCompatiblePiiProviderError(
          ...
          "PII model reported findings but returned no transformedText"
        );
      }
      return {
  ...
  private async callChat<TResponse>(
    ...
    try {
      ...
      // Every OpenAiCompatiblePiiProviderError represents a real provider problem
      // (timeout / network / 5xx / malformed response). Count them all.
      // Non-provider errors (programmer errors, signal aborts from caller
      // unrelated to provider health) shouldn't count.
      if (error instanceof OpenAiCompatiblePiiProviderError && error.code !== "breaker_open") {
  ...
  private async doCallChat<TResponse>(
    ...
    try {
      ...
      if (controller.signal.aborted && !externalSignal?.aborted) {
        throw new OpenAiCompatiblePiiProviderError("timeout", `PII model request timed out after ${this.timeoutMs}ms`);
      }
      throw new OpenAiCompatiblePiiProviderError(
        "network_error",
    ...
    try {
      ...
      throw new OpenAiCompatiblePiiProviderError(
        "invalid_json",
        `PII model response was not valid JSON: ${error instanceof Error ? error.message : String(error)}`
      );
    }
  }
}
