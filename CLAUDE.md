...
1. `POST /messages` (`routes/messages.ts`) validates input, enforces rate limits and quotas, resolves the model via `resolveRuntimeModel`, and calls `streamAssistantReplyAGUI` (`services/sse-stream-writer-agui.ts`).
2. `streamAssistantReplyAGUI` hijacks the raw socket, sets SSE headers, creates a `ToolExecutionContext`, and calls `runtimeAdapter.runMessageAGUI`.
3. `DeepAgentsRuntimeAdapter` (`services/deep-agents/deep-agents-runtime-adapter.ts`) lazily builds a per-session deepagentsjs agent (`createDeepAgent`) and runs the turn as a LangGraph `streamEvents` v2 stream in the backend process, returning an `AsyncIterable<BaseEvent>`.
4. `stream-events-to-agui.ts` maps framework events directly to AG-UI. The writer streams those events to the browser and incrementally persists assistant text and tool results. The scheduler consumes the same stream.
...
- `stream-events-to-agui.ts` — maps LangGraph `streamEvents` v2 envelopes directly to AG-UI. Subagent namespaces (multi-segment `langgraph_checkpoint_ns`) are dropped; `write_todos` emits `STATE_DELTA`; MCP card server attribution comes from the load-time toolName-to-serverId map.
...
- **Turn input is text-only.** `RuntimeUserInput` is `{ type: "text"; text: string }` — there is no image content block and no unsupported-image notice; both were removed with the retired runtimes. An uploaded image artifact is never sent to the model as an image: `turn-input-builder.ts` lists it by name, id, mime type and workspace path, and `artifact-workspace-sync.ts` writes the file into `./artifacts/` in the sandbox, so the agent reaches it with the file/`execute` tools. A real multimodal path needs LangChain content blocks plus a model-capability check, and does not exist yet.
...
**Native HITL approvals** are LangGraph **interrupt-based**: `tenant_settings.approval_policy` (`"never"` bypasses gating) drives an `interruptOn` map built in `deep-agents-graph.ts` — all MCP gateway tools (minus read-only ones when `auto_approve_read_only_tools` is on) plus the mutating built-ins (`execute`, `write_file`, `edit_file`), plus the read-only built-ins when the read-only bypass is off. An interrupt pauses the graph BEFORE tool execution and checkpoints; the adapter detects pending interrupts after the stream ends, persists an approval row, emits an AG-UI `CUSTOM` event named `approval_required`, and resumes with a `Command` once decided. **Resume is keyed per interrupt id** so concurrent interrupts (parallel `task` subagents each hitting a gated tool) get their own decisions — do not collapse this to a flat decisions array. The frontend calls `POST /approvals/:approvalId/decision` with body `{ decision: "approve" | "reject", rememberForTurn?: boolean }` (`routes/approvals.ts`).

Policy Center `require_approval` uses the runtime's checkpointed graph interrupt. After approval, the runtime injects a call-bound `policyApprovalId`; the gateway proceeds only when its approval row matches the same session, user, tool, server, tool context, and canonical argument hash. Missing or mismatched proof is denied. Scheduled turns use this same durable pause instead of a special-case denial.

Pending approvals carry a wall-clock TTL (`APPROVAL_REQUEST_TTL_MS`, default 10 min). On expiry the DB row moves to `status='expired'`, an `approval.expired` audit event is written, an AG-UI `CUSTOM` event named `runtime_notice` (`noticeId = approval-expired:<approvalId>`) is pushed to the active turn, and the paused graph is resumed with a reject. The row also carries a DB-level `expires_at` so a process death still lets the startup sweep recover it.
...
| Tenant settings | `TenantSettingsStore` (`tenant-settings-store.ts`) | One row per tenant. Controls `enabledToolIds`, `enabledMcpServerIds`, `approvalPolicy`, `approvalReviewer`, `autoApproveReadOnlyTools`, `policyEnforcementMode`, `allowCommandExecution`, `developerInstructions`, `webSearchMode`, and `showEffortSelector`. The `system` tenant's row acts as the platform default. |

`allowCommandExecution` is owner-controlled security posture. Admins can edit other Agent Settings, but the backend rejects changes to this field and strips unchanged full-form values before persistence so stale admin forms cannot overwrite an owner's decision.

Policy Center rules live in `policy_rule` and are evaluated before and at the MCP gateway. Active dimensions are `toolNames`, `categories` (managed-tool domain or proxy MCP server id), `severities`, and `turnContexts`; effects are `allow`, `require_approval`, and `block`. `tenant_settings.policy_enforcement_mode` is the tenant-level monitor/enforce switch.
